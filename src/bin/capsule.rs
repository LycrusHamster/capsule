...
fn run_cli() -> Result<()> {
    ...
    let mut app = App::new("Capsule")
        ...
        .subcommand(SubCommand::with_name("build").about("Build contracts")
            ...
            .arg(Arg::with_name("env").long("env").short("e").multiple(true).takes_value(true)/*.value_delimiter(";")*/.help("Environment variable pass to compiler, only ASCII key and value pairs, separated by equal-signs"))
            .display_order(3))
